...
for (let i = 1; i < lines.length; i++) {
    ...
    if (emailKey !== 'lukasleivalukas123456789@gmail.com') continue;
    ...
    if (date) {
        ...
        if (hasNewData) {
            console.log(`Row ${i} has new data for date: ${date}`);
            const existingEvalIndex = athleteData.evaluations.findIndex(e => e.date === date);
            if (existingEvalIndex >= 0) {
                console.log(`Merged with existing eval for date: ${date}`);
            } else {
                athleteData.evaluations.push(newEval);
                console.log(`Pushed new eval for date: ${date}`);
            }
        } else {
            console.log(`Row ${i} has NO new data for date: ${date}`);
        }
    }
}
console.log(JSON.stringify(athletesCache['lukasleivalukas123456789@gmail.com'], null, 2));
